...
if [ "$(expr $- : '.*i')" -ne 0 ]; then
  ...
  if [ -z "$ZSH_VERSION" ]; then
      ...
      # hostname (and location if configured) in orange
      if [ -f "/etc/machine-info" ] && [ ! -z "$(sed -n -e 's/LOCATION=//p' /etc/machine-info)" ]; then
        PS1+="\
        host=\"\${ORANGE}\H[$(sed -n -e 's/LOCATION=//p' /etc/machine-info)]\${WHITE}\" ; "
      else
        PS1+="\
        host=\"\${ORANGE}\H\${WHITE}\" ; "
      fi
